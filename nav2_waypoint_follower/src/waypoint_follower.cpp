...
namespace nav2_waypoint_follower
{
  ...
  WaypointFollower::on_configure(const rclcpp_lifecycle::State & /*state*/)
  {
    ...
    callback_group_or_ = create_callback_group(
        rclcpp::CallbackGroupType::MutuallyExclusive,
        false);
    callback_group_executor_or_.add_callback_group(callback_group_or_, get_node_base_interface());
    publisher_ = create_publisher<std_msgs::msg::String>("testTopic", 10);
    ...
    orientation_client_ = rclcpp_action::create_client<ClientOr>(
        get_node_base_interface(),
        get_node_graph_interface(),
        get_node_logging_interface(),
        get_node_waitables_interface(),
    ...
    while (rclcpp::ok())
    {
      ...
      else if (current_goal_status_ == ActionStatus::SUCCEEDED)
      {
        RCLCPP_INFO(
            ...
            goal_index);
        //orientation manager start
        // nav2_msgs::action::Orientation::Goal goal_msg; 
        ...
        // auto send_goal_options_or = rclcpp_action::Client<nav2_msgs::action::Orientation>::SendGoalOptions();
        // send_goal_options_or.goal_response_callback =
        ...
        // while (current_goal_status_or_ == ActionStatus::PROCESSING)
        // {
        //   /* code */
        //   if (!orientation_client_->wait_for_action_server(std::chrono::seconds(10)))
        // {
  ...
  void
  WaypointFollower::resultCallbackOr(
      const rclcpp_action::ClientGoalHandle<ClientOr>::WrappedResult &result)
  {
    RCLCPP_INFO(
        get_logger(),
        "result response");
    ...
    {
    case rclcpp_action::ResultCode::SUCCEEDED:
      current_goal_status_or_ = ActionStatus::SUCCEEDED;
      return;
    case rclcpp_action::ResultCode::ABORTED:
      current_goal_status_or_ = ActionStatus::FAILED;
      return;
    case rclcpp_action::ResultCode::CANCELED:
      current_goal_status_or_ = ActionStatus::FAILED;
      return;
    default:
      current_goal_status_or_ = ActionStatus::UNKNOWN;
      return;
    }
  }
  ...
  void
  WaypointFollower::goalResponseCallbackOr(
      const rclcpp_action::ClientGoalHandle<ClientOr>::SharedPtr &goal)
  {
    RCLCPP_INFO(
        get_logger(),
        "goooal response");
